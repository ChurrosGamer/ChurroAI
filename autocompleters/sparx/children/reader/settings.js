const { ButtonBuilder, ButtonStyle, ActionRowBuilder, MessageFlags, ContainerBuilder, TextDisplayBuilder, ModalBuilder, TextInputBuilder, TextInputStyle, SeparatorBuilder, SeparatorSpacingSize, ThumbnailBuilder, SectionBuilder } = require('discord.js');
...
function getContainer(data, disabled=false) {
    ...
    sectionButtons.push(
        new SectionBuilder().addTextDisplayComponents(new TextDisplayBuilder().setContent(`⭐️ **Sparx Reader Points**: ${data.srp}`))
            .setButtonAccessory(new ButtonBuilder().setLabel('⭐️ SRP').setCustomId('srp').setStyle(ButtonStyle.Primary).setDisabled(disabled)));
...
async function handleSetting(interaction, data) {
    ...
    collector.on('collect', async (componentInteraction) => {
        if (componentInteraction.isButton()) {
            if (componentInteraction.customId === 'srp' || componentInteraction.customId === 'wpm') {
                const isPoints = componentInteraction.customId === 'srp';
                const modal = new ModalBuilder()
                    .setCustomId(`autocompleterModule_sparx(reader)_${isPoints ? 'srp' : 'wpm'}Change_settings`)
                    .setTitle(`${isPoints ? 'Point' : 'WPM'} Setting`);
                const input = new TextInputBuilder()
                    .setCustomId(isPoints ? 'srp' : 'wpm')
