...
async function parser(apikeysUser, data, activityIndex, questionIndex, model, interaction, incorrect_answers) {
    const parsedData = parseQuestion(data);

    const { geminiAnswer, geminiAnswers } = require('../../../../gemini/sparx_maths/main');
    const apikeys = removeDuplicates([...apikeysUser, ...(await getApiKeys())]);
    let aiAnswered;
