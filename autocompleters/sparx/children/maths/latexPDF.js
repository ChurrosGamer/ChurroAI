...
async function downloadImage(url, filename) {
    try {
        ...
        try {
            ...
            await fs.writeFile(filename, Buffer.from(arrayBuf));
...
async function convertToPDF(obj, packageID, showWorkingOut = false, showQuestion = false) {
    // Ensure image directory exists dynamically
...
async function cleanup(packageID) {
    ...
    for (const f of auxFiles) {
        if (await fileExists(f)) try { await fs.unlink(f); } catch {}
    }

    if (await fileExists(imgDir)) {
        const files = await fs.readdir(imgDir);
        for (const file of files) {
            if (file !== '.gitkeep') {
                try { await fs.unlink(path.join(imgDir, file)); } catch {}
            }
        }
    }
}
